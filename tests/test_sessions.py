...
drops the dead ones; cloud mode refuses to sign anything without an explicit secret.
"""
...
def test_revoke_all_spares_the_session_pressing_the_button(db, secret):
    ...
    assert [s["id"] for s in sessions.live_for(db, "local", at=T0)] == [c, b, a] or \
        {s["id"] for s in sessions.live_for(db, "local", at=T0)} == {a, b, c}
...
def test_purge_drops_expired_and_long_revoked_sessions(db, secret):
    ...
    db.execute("UPDATE sessions SET revoked_at=? WHERE id=?", ((T0 - timedelta(days=10)).isoformat(), gone))
    assert sessions.purge(db, at=T0) == 2
    assert {r[0] for r in db.execute("SELECT id FROM sessions")} == {keep}
