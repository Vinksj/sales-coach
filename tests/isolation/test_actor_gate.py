...
A request with no valid session is answered before any store is opened (a page is sent to /login,
anything else gets 401); a session for a disabled user gets 403 and counts for nothing; a session
for a user who does not exist is no session; open paths pass with nobody bound and read the store
as system; and a route that runs a statement with nobody bound trips the suite's assertion
(db.NoActorBound) instead of silently reading nothing.
"""
import pytest
from fastapi.testclient import TestClient

from salescoach import hosted, identity, users
...
def _cookie(user_id):
    return {hosted.COOKIE: hosted.issue_session(user=user_id)}
...
def test_a_disabled_users_session_gets_403_and_binds_nobody(client):
    r = client.get("/", headers={"accept": "text/html"}, cookies=_cookie("u-gone"))
    assert r.status_code == 403
    assert client.get("/loops.json", cookies=_cookie("u-gone")).status_code == 403
    assert client.get("/deals", headers={"accept": "text/html"}, cookies=_cookie("u-gone")).status_code == 403


def test_a_session_for_an_unknown_user_is_no_session(client):
    r = client.get("/", headers={"accept": "text/html"}, cookies=_cookie("u-nobody"))
    assert r.status_code == 303 and r.headers["location"].startswith("/login")
    assert client.get("/loops.json", cookies=_cookie("u-nobody")).status_code == 401


def test_an_active_user_is_bound_and_health_is_open(client):
    assert client.get("/", headers={"accept": "text/html"}, cookies=_cookie("u-a")).status_code == 200
    me = client.get("/me/setup", headers={"accept": "text/html"}, cookies=_cookie("u-a"))
...
def test_a_route_that_reads_with_nobody_bound_trips_the_assertion(client):
    ...
    with pytest.raises(dbmod.NoActorBound):
        client.get("/__test/no-actor", cookies=_cookie("u-a"))
    assert client.get("/__test/system-read", cookies=_cookie("u-a")).json() == {"ok": 1}
